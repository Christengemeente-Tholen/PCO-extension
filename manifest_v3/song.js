...
function buttonStyle(newButton, response) {
    ...
    if (buttonList.querySelector("#pco_extension-button") === null) {
      ...
      if (buttonList.querySelector("#pco_extension-button") === null) {
        ...
        newButton.onclick = async () => {
          ...
          const planningCenterSettings = await chrome.storage.local.get(["autoremoveHeading", "autoremoveHeadingItems"])
        ...
        buttonStyle(newButton, response);
        buttonList.appendChild(newButton);
      }
    }
    if (buttonList.querySelector("#pco_extension-ccli_button") === null) {
      const response = await chrome.runtime.sendMessage({ script: "ccli", song: result[1] });
      if ("error" in response) {
        console.error(response.error);
      }
      const data = response?.data;
